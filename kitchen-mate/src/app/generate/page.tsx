...
const GeneratePage = () => {
  const [addToMyRecipeStatus, setAddToMyRecipeStatus] = useState<string | null>(
    null
  );
  const [generatedRecipeId, setGeneratedRecipeId] = useState(null);
  const [loading, setLoading] = useState(false);

  const [promtIngredients] = useState(
    "Make recommendations for 1 food recipes based on the following ingredients:"
  );
  ...
  const [promtFind] =
    useState(`, Provide it in JSON answer format, which consists of properties: 
    ...
    try {
      ...
      const response = await axios.post(
        process.env.NEXT_PUBLIC_BASE_URL +
          `/api/wishlists/${generatedRecipeId}`,
        {
